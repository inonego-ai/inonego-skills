...
이 파일들은 `skills/` 아래의 스킬이 아니다. 일반 스킬 탐색 과정에서 자동으로 읽거나 불러오지 않는다.

## 파일

- `CHATGPT_WEB_BOOTSTRAP.template.md`
  - 로컬 작업 세션의 공통 초기화 절차 템플릿
  - 전역/프로젝트 지침 탐색, 스킬 머리말 메타데이터 확인과 지연 로드를 정의한다.
- `CUSTOM_INSTRUCTIONS.template.md`
  - ChatGPT Web 맞춤 지침에 넣을 짧은 부트스트랩 경로 안내 템플릿

## 사용 방법

1. `CHATGPT_WEB_BOOTSTRAP.template.md`를 로컬의 별도 에이전트 컨텍스트 디렉터리에 복사한다.
2. `<GLOBAL_AGENT_INSTRUCTIONS>`, `<GLOBAL_SKILL_ROOT_*>` 같은 자리표시자를 현재 환경의 실제 경로로 바꾼다.
...
실사용 부트스트랩은 이 마켓플레이스 저장소 내부에 두지 않는 것을 권장한다. 이 저장소에는 재사용 가능한 템플릿만 보관하여, 실제 Web 부트스트랩이 자기 자신의 템플릿을 지침이나 스킬로 중복 발견하지 않도록 한다.
